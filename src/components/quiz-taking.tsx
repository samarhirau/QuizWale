...
//     if (!quiz || !answers) return { score: 1, percentage: 0, totalQuestions: 0 }
...
const startQuiz = () => {
  ...
  const handleAutoSubmit = async () => {
    recordQuestionTime()
    await submitQuiz()
    toast("Time's up! Quiz has been automatically submitted.")
  }
...
const submitQuiz = async () => {
  ...
  try {
    ...
    const finalQuestionTimes = recordQuestionTime()
  ...
  <div className="text-red-500 text-sm">No score data found</div>
              ...
              <div className="text-center">
                <div className="text-3xl font-bold text-purple-600">{formatTime(results?.submission?.timeSpent || 0)}</div>
                <div className="text-sm text-muted-foreground">Time Taken</div>
              </div>
            </div>
